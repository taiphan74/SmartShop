...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    @Query(value = "SELECT p.id AS id, " +
           ...
           "FROM Product p JOIN p.shop s " +
           "WHERE p.status = true AND s.status = 'APPROVED'",
           countQuery = "SELECT COUNT(p.id) FROM Product p JOIN p.shop s WHERE p.status = true AND s.status = 'APPROVED'")
    Page<ProductProjection> findPublicProductsFromApprovedShops(Pageable pageable);
    ...
    @Query(value = "SELECT p.id AS id, " +
           ...
           "WHERE p.status = true AND s.status = 'APPROVED' " +
           "AND (c.path = :categoryPath OR c.path LIKE CONCAT(:categoryPath, '/%'))",
           countQuery = "SELECT COUNT(p.id) FROM Product p JOIN p.shop s JOIN p.category c " +
           "WHERE p.status = true AND s.status = 'APPROVED' " +
           "AND (c.path = :categoryPath OR c.path LIKE CONCAT(:categoryPath, '/%'))")
    Page<ProductProjection> findPublicProductsByCategoryPath(String categoryPath, Pageable pageable);
